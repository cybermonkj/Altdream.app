            ...
            <div class="row justify-content-center">
                <div class="col-lg-6">
                    <div class="login-form-wrapper my-5 py-5">
                        <h2>{{__('Verify Your E-Mail')}}</h2>
                        <x-msg.error/>
                        <x-msg.success/>
                        <div class="alert alert-warning alert-block">
                            <button type="button" class="close" data-dismiss="alert">×</button>
                            <strong>{{ __('A verification code has been sent to your email address') }}</strong>
                        </div>
                        <form action="{{route('user.email.verify')}}" method="post" enctype="multipart/form-data" class="account-form verify-mail">
                            @csrf
                            <div class="form-group">
                                <input type="text" name="verification_code" class="form-control" placeholder="{{__('Enter code here')}}">
                            </div>
